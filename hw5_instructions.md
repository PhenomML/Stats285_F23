...
So far, we tried to keep our code simple. Simplicity is essential. Debugging a server is quite a bit more difficult than debugging code on your laptop; debgging a parallel server, unsurprisingly, more so. Hence, we organize things so you perfect your code on your laptop and then apply it *without change* at a massively greater scale. To this end, we introduced simple code that created the results of a **1000 x 1000** SVD of noisy data. We then repackaged it as a function that takes keyword arguments and returns a DataFrame. This code we ran on both your laptop and a server. 

That brings us to our fourth computer activity -- computing with a cluster to (approximately) do (part of) a **1,000,000 x 1,000** SVD, within the divide and recombine (map-reduce) paradigm. The same basic code we used before is re-used to make a mapper function, applied in 1,000 separate `map' instances, each of size 1000 x 1000. Each `map' instance saves relevant outputs to the cloud. Once the `map' outputs are in the cloud, we will eventually do the `reduce' step, using a Jupyter or Colab notebook. 

To do the `map' job distribution into 1000 instances, we follow the advice of the Stanford Research Computing Center (SRCC) staff to use the `sbatch array` command on the command line. Like `sbatch', `sbatch array' launches jobs; unlike simple `sbatch', this single command can launch many instances of `map_function.py` from the command line. Each instance saves its results to a remote database, hosted at Google BigQuery [GBQ]. 

We found that straightforward use of the `sbatch array` command isn't a good fit for database centric cloud computing. To make  the scripts work reliably the `map' functions first writes the data as files and then later writes the amalgamated data to the remote database. Hence, there are three functions involved: `map_function.py`, `gather_csv_to_gbq.py`, and `HW5_analysis.ipynb`. `map_function.py` creates a bunch of `.csv` files; those are later read by `gather_csv_to_gbq.py` and sent to Google BigQuery. This experiment is driven by the `sbatch` script `hw5_array.sh`. 

To replace your burden on gathering computational results, we are going to start using a simple system to run many independent tasks and write the results to the remote database. This system, [EMS or Experiment Management System](https://github.com/adonoho/EMS), takes a definition of your parameters as arrays and then calls your function with the combination of all parameter values. It then stores the result of each function in a database. 

Because modern laptops contain many processors, the one I am writing this on has 6 symmetric cores and can run 12 concurrent tasks, you can frequently run your code quite a bit faster than normal using EMS. EMS depends upon the Python system `Dask' which deploys jobs and can. As our tasks are embarrassingly parallel, we will not be exploiting Dask for other than processor/thread/worker management. But it is available for more advanced work. For example, there is an advanced way to use Dask to solve the [Tall & Skinny SVD problem](https://examples.dask.org/machine-learning/svd.html#Compute-SVD-of-Tall-and-Skinny-Matrix).
